...
- FastAPI 常驻自动任务：赛程、积分榜、证据同步、赛后结算和赛前模型分析（`AUTOMATION_ANALYSIS_ENABLED` 默认开启，可关闭后完全改为管理员手动触发）
...
- 球员身价来自懂球帝历史身价（`DongqiudiPlayerValueProvider`，带来源与时效，授权变化时可替换）；暂无身价数据时显示“暂无可靠身价”，不抓取 Transfermarkt 或伪造数据
