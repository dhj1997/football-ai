...
export function OperationsPanel() {
  ...
  return (
    <section className="operations-panel" aria-labelledby="operations-title">
      <SectionHeader className="team-section-heading" eyebrow="AUTOMATION RUNS" title="常驻作业" titleId="operations-title" meta={enabled ? (analysisEnabled ? "自动运行中" : "自动同步中 · 预测手动") : "自动运行已关闭"} />
      ...
      <div className="operations-grid">
        {jobs.map((job) => {
          ...
          return <article key={job.key}>
            <div><span>{label}</span><StatusBadge className={`run-${item?.status ?? "none"}`} variant={statusVariant(item?.status)}>{statusText(item?.status)}</StatusBadge></div>
