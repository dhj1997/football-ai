import type { BankrollSummary, DateFilter, DecisionAudit, Fixture, FixtureDetail, FixtureLeagueFilter, ModelEvaluationResponse, ModelKey, PredictionMetrics, RuntimeConfigResponse, SimulatedBet, StandingsResponse, StrategyPerformance, TeamDetailResponse } from "./types";
...
export function readCachedFixtures(date: DateFilter, league: FixtureLeagueFilter): FixtureListResponse | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(fixtureCacheKey(date, league));
    if (!raw) return null;
    const cached = JSON.parse(raw) as CachedFixtureList;
    return cached.response?.items && Array.isArray(cached.response.items) ? cached.response : null;
  } catch {
    return null;
  }
}
...
function hasFreshFixtureCache(date: DateFilter, league: FixtureLeagueFilter) {
  ...
  try {
    ...
    if (!raw) return false;
    const cached = JSON.parse(raw) as CachedFixtureList;
    return Number.isFinite(cached.cached_at) && Date.now() - cached.cached_at < fixtureCacheTtlMs;
  } catch {
    return false;
  }
}

function writeCachedFixtures(date: DateFilter, league: FixtureLeagueFilter, response: FixtureListResponse) {
  ...
  try {
    ...
    const compact: FixtureListResponse = {
      ...response,
      items: response.items.map(({ id, provider_id, fixture_date, league_key, league, kickoff, status, home_team, away_team, score, venue, lineup_confirmed, is_demo, evidence_summary, has_prediction }) => ({
        id, provider_id, fixture_date, league_key, league, kickoff, status, home_team, away_team, score, venue, lineup_confirmed, is_demo, evidence_summary, has_prediction,
      })),
    };
    window.localStorage.setItem(fixtureCacheKey(date, league), JSON.stringify({ cached_at: Date.now(), response: compact } satisfies CachedFixtureList));
  } catch {
    // Storage is optional; quota and privacy-mode failures must not affect live loading.
  }
}

export function prefetchFixtures(date: DateFilter, league: FixtureLeagueFilter) {
...
export async function fetchFixtures(date: DateFilter, league: FixtureLeagueFilter): Promise<FixtureListResponse> {
...
export async function fetchRuntimeConfig(): Promise<RuntimeConfigResponse> {
  return readJson(await fetch("/api/admin/model-config", { cache: "no-store" }));
}

export async function updateRuntimeConfig(payload: unknown): Promise<RuntimeConfigResponse> {
  return readJson(await fetch("/api/admin/model-config", {
    method: "PUT",
