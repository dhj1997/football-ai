      ...
      {teams.map(({ side, name, data }) => {
        ...
        return <div className={`player-impact-team ${side}`} key={side}>
          ...
          <div className="impact-player-groups">
            <div><span>关键可用</span><ul>{data.key_available_players.length ? data.key_available_players.map((player) => <li key={impactPlayerKey(player)}><b>{player.name}</b><small>{player.player_role} · 预计 {Math.round(player.expected_minutes)} 分钟</small></li>) : <li><small>暂无可靠识别</small></li>}</ul></div>
            <div><span>关键缺阵</span><ul>{data.key_absent_players.length ? data.key_absent_players.map((player) => <li key={impactPlayerKey(player)}><b>{player.name}</b><small>{player.player_role} · 影响 {percent(player.absence_impact ?? 0)}</small></li>) : <li><small>暂无关键缺阵</small></li>}</ul></div>
          </div>
...
export function ProbabilityPanel({ prediction, fixture, bet, onManualPredict, predicting = false }: { prediction: Prediction; fixture: Fixture; bet: SimulatedBet | null; onManualPredict?: () => void; predicting?: boolean }) {
  ...
  const usesCurrentPrompt = prediction.ai?.prompt_version === "football-forecast-v3";
  const executionLabel = decision?.status === "bet" ? "执行模拟下注" : decision?.status === "no_bet" ? "本场不下注" : "数据不足，不执行";
  return (
    <section className="prediction-panel" aria-labelledby={headingId}>
      ...
      {prediction.ai && (
        <div className={`ai-assessment ai-${prediction.ai.status}`}>
          <div className="ai-assessment-copy">
            ...
            <strong>{prediction.ai.status !== "completed" ? "AI 不可用，当前仅显示 Poisson 基线" : usesCurrentPrompt ? prediction.analysis_summary : "历史模型概率已保留；球员影响与执行决定已按当前规则重新计算。"}</strong>
            <small>{prediction.ai.status === "completed" ? `${prediction.ai.returned_model} · ${usesCurrentPrompt ? `${prediction.ai.prompt_version} · ${prediction.ai.evidence_version ?? "证据版本待确认"}` : `历史提示词 ${prediction.ai.prompt_version ?? "版本未知"}`}` : prediction.ai.error}</small>
            {usesCurrentPrompt && prediction.player_analysis?.replacement_gap ? <p className="ai-thesis"><b>替补差值</b>{prediction.player_analysis.replacement_gap}</p> : null}
          </div>
          <dl>
          ...
          {usesCurrentPrompt && prediction.risk_factors?.length ? <div className="ai-risk-factors"><span>风险因素</span><ul>{prediction.risk_factors.map((risk) => <li key={risk}>{risk}</li>)}</ul></div> : null}
          {usesCurrentPrompt && prediction.missing_evidence?.length ? <div className="ai-caveats"><span>证据缺口</span><ul>{prediction.missing_evidence.map((item) => <li key={item}>{item}</li>)}</ul></div> : null}
          {aiHandicap && <p className="ai-handicap-note">亚洲让球覆盖预测：{aiHandicap.line !== null ? `${handicapDirection(aiHandicap.line, fixture.home_team.name, fixture.away_team.name)} · 主队 ${percent(aiHandicap.home_cover_probability ?? 0)} / 客队 ${percent(aiHandicap.away_cover_probability ?? 0)}` : "无可用盘口"}{aiHandicap.reason ? ` · ${aiHandicap.reason}` : ""}</p>}
      ...
      <section className="market-value-layer" aria-label="赔率价值">
        ...
        {marketRows.length ? <div className="market-value-table"><div className="market-value-head"><span>市场</span><span>模型</span><span>回本线</span><span>去水</span><span>优势</span></div>{marketRows.map((row) => <div className="market-value-row" key={`${row.market}-${row.selection}`}><span className="market-value-name"><b>{selectionText(row.selection)}</b><small>{row.market === "asian_handicap" && row.line !== undefined ? `亚洲盘 ${row.line > 0 ? "+" : ""}${row.line}` : "胜平负"} · {row.price.toFixed(2)}</small></span><span><small>模型</small><b>{percent(row.model_probability)}</b></span><span><small>回本线</small><b>{percent(row.break_even_probability)}</b></span><span><small>去水</small><b>{percent(row.de_vig_probability)}</b></span><span className={row.expected_edge > 0 ? "edge-positive" : "edge-negative"}><small>优势</small><b>{row.expected_edge > 0 ? "+" : ""}{(row.expected_edge * 100).toFixed(1)}%</b></span></div>)}</div> : <p className="data-empty">没有可计算的匹配赔率市场</p>}
      </section>
      <section className={`execution-layer execution-${decision?.status ?? "insufficient_data"}`} aria-label="执行决定">
        <header><span>03 · 执行决定</span><strong>{executionLabel}</strong><small>后端确定性规则 · 仓位上限 2%</small></header>
        <div><p>{decision?.reason ?? "当前预测版本缺少确定性决策结果"}</p>{decision?.reason_codes?.length ? <ul>{decision.reason_codes.map((code) => <li key={code}>{decisionReasonLabels[code] ?? code}</li>)}</ul> : null}<dl><div><dt>考察市场</dt><dd>{marketText(decision?.considered_market ?? undefined)}</dd></div><div><dt>预期优势</dt><dd>{decision?.expected_edge !== null && decision?.expected_edge !== undefined ? `${decision.expected_edge > 0 ? "+" : ""}${(decision.expected_edge * 100).toFixed(1)}%` : "-"}</dd></div><div><dt>不确定性</dt><dd>{percent(decision?.uncertainty ?? 1)}</dd></div><div><dt>模拟仓位</dt><dd>{percent(decision?.stake_fraction ?? 0)}</dd></div></dl></div>
      </section>
      {bet && (
    ...
    <section className="dual-prediction-section" aria-label="双模型初步预测">
      <div className="dual-prediction-heading">
        ...
        <div className="dual-prediction-actions">
          <small>共享同一证据与 PromptContract，执行决定由后端统一计算</small>
      ...
      <div className="dual-prediction-shell">
        ...
        {entries.map(([key, prediction]) => (
          ...
          >
            ...
            <span className={`model-tab-investment${prediction.decision?.status !== "bet" ? " no-bet" : ""}`}>{prediction.decision?.status === "bet" ? selectionText(prediction.decision.selection) : prediction.decision?.status === "no_bet" ? "本场不下注" : "数据不足"}</span>
